# Xentient: Repurpose — The IoT Terminal for Any AI Brain

> **Date:** 2026-04-19 (revised)
> **Status:** Final design vision (post-demo architecture)
> **Scope:** NOT implemented before Apr 24 demo. This document captures the architectural reframe and integration strategy for post-capstone evolution.
> **Author:** sarmi + AI co-pilot
...
| Tool Router (thin) | Pack-driven dispatcher that routes tool calls to the right adapter |
...
```
┌─────────────────────────────────────────────────────────────────┐
│  AI Brain Layer (runs on server — local or cloud)               │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐    │
...
│  │  - Receives audio/text from Xentient                    │    │
│  │  - Routes to active AI brain                            │    │
│  │  - Returns audio/text/actions to Xentient              │    │
│  └──────────────────────────┬───────────────────────────────┘    │
└─────────────────────────────┼────────────────────────────────────┘
                              │  WebSocket / MQTT / HTTPS
                              │  (runs on same LAN or remote server)
┌─────────────────────────────┼────────────────────────────────────┐
│  Xentient Harness ("terminal OS") — what we build                │
│                                                                  │
│  ┌──────────┐  ┌──────────┐  ┌───────────┐  ┌────────────────┐  │
│  │ Voice I/O│  │ MQTT/    │  │ LCD Face  │  │ Mode/Space    │  │
│  │ Pipeline │  │ Sensor   │  │ State     │  │ Manager       │  │
│  │ (own)    │  │ Bridge   │  │ Machine   │  │ (sleep/listen │  │
│  │          │  │ (own)    │  │ (own)     │  │ /active/record│  │
│  └─────┬────┘  └────┬─────┘  └─────┬─────┘  └──────┬────────┘  │
│        └──────┬──────┴──────────────┴───────────────┘           │
│               │                                                   │
│        ┌──────┴──────┐                                           │
│        │ Tool Router │  ← pack-driven, space-gated               │
│        │ (thin)      │                                           │
│        └──┬───┬───┬──┘                                           │
│           │   │   │                                              │
│    ┌──────┘   │   └──────┐                                       │
...
The **Mode Manager** is a new core component. It persists the current mode, manages transitions, and routes audio/processing based on mode:
...
- **Space-scoped facts:** "living-room temperature is usually 24°C" — stored with `space_id` tag
...
**What it replaces:**
- `memory/MemoryDB.ts` → DELETE
- `memory/FactExtractor.ts` → DELETE
- `memory/MemoryInjector.ts` → DELETE
- `memory/schema.ts` → DELETE
...
The AI Brain Layer and Xentient Harness communicate through a **communication layer** that can run anywhere:
...
└──────────────────┘                        │
                                             │
┌────────────────────────────────────────────┘
│  Xentient Harness (same server or remote)
│  - Voice pipeline
│  - MQTT bridge to Node Base
│  - LCD face state machine
│  - Mode/Space manager
│  - Pack loader
│  - Tool Router → routes to adapters
```
...
**Key principle:** The harness doesn't care where the brain runs. It just needs a WebSocket/REST connection. Local for privacy, cloud for power.

---

## Tool Router: Pack-Driven, Space-Gated

The Tool Router is the **glue layer** between Xentient's core and the AI brain layer. It's thin — it doesn't do reasoning, it dispatches.
...
Folder spec, manifest schema, lifecycle, hot-reload, MQTT control — all preserved from NOTES.md. New handler types added (see Tool Router table above).
...
|-------|------|-----------|-------------|
| **P1: Hermes Adapter** | Replace custom LLM+memory loop with Hermes connection. Delete `memory/` dir. Add `HermesAdapter.ts`. | -300 LOC custom, +80 LOC adapter | Hermes Agent installed on server |
...
| **P6: OpenClaw Adapter** | Add `OpenClawAdapter.ts` for computer-use handler. | +60 LOC | P5 complete |
| **P7: Archon Adapter** | Add `ArchonAdapter.ts` for agent-delegate handler. | +50 LOC | P5 complete |
| **P8: Communication Bridge** | REST/WS/MQTT bridge between harness and AI brain layer. Configurable local/cloud. | +100 LOC | P1 complete |

### Target Harness Size

**Current:** ~600 LOC (Pipeline + Memory + Providers + Comms + Index)
**After P1-P8:** ~730 LOC (Pipeline + Comms + Tool Router + Space/Mode Mgr + Pack Loader + thin adapters)

The harness shrinks dramatically because custom memory code drops out (-300 LOC) and Hermes handles the entire LLM+memory+skills+reasoning loop. It grows slightly for new components. Net: ~130 LOC increase, but infinitely more capable because Hermes provides months of work for free.
...
| NEW: `engine/ToolRouter.ts` | **CREATE** — pack+space-driven handler dispatch |
...
| NEW: `bridge/CommunicationBridge.ts` | **CREATE** — REST/WS/MQTT bridge to AI brain layer |
...
> "Xentient today is a voice-first IoT assistant with basic memory. After demo, it becomes a terminal — any AI brain can plug into this body. Hermes gives it full intelligence. Mem0 gives it persistent memory that doesn't rot. OpenClaw gives it computer use. The harness goes from 600 to 730 lines, but the capability jumps from a chatbot to a room-scale intelligence platform."

---

## Open Questions

1. **Hermes deployment: local or cloud?** Local (Docker on Pi/computer) for privacy. Cloud for dev/testing. Both supported by the architecture.
2. **Mem0 self-hosted vs. cloud?** Self-hosted (Docker) for privacy — user data stays local. Cloud for dev/testing. Hermes connects to either.
3. **Communication protocol between harness and Hermes?** Hermes exposes REST + WebSocket APIs. The `HermesAdapter` uses WebSocket for streaming, REST for tool calls.
4. **OpenClaw sandbox isolation?** OpenClaw runs its own Docker sandbox. Xentient doesn't need to add sandboxing on top.
5. **Space persistence?** Space config lives in `harness/config/spaces.json`. Hot-reload like packs.
6. **Multi-Node Base demo?** Single Node Base for Apr 24 demo. Multi-space is post-demo P4.
7. **Basic mode LLM quality?** Direct provider call with no memory/context. Good for quick interactions, but Hermes mode is the intended experience.
...
- **Tool Router:** pack+space-driven dispatcher that routes tool calls to the correct adapter (Hermes, OpenClaw, Archon, basic LLM)
...
- **Sidecar:** an external process managed by the harness, communicating via local API
- **Basic mode:** direct LLM provider call with no memory or skills — always available as fallback
- **Communication Bridge:** the layer between Xentient harness and AI brain services, configurable for local or cloud deployment
- **Context rot:** stale/irrelevant memory leaking into LLM context — solved by Mem0's recency weighting

---

*This document supersedes the post-demo sections of NOTES.md. All B1-B7 hardware/firmware decisions remain authoritative in NOTES.md. The reframe applies to software architecture only, post-Apr 24.*